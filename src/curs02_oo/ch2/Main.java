...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("Get all studio names in witch plays actor with name \"actor cu 2 oscaruri\":");
        System.out.println(getAllStudioInWitchPlaysActorWithXName(studioDatabase, "actor cu 2 oscaruri"));
    ...
    public static List<String> getAllStudioNamesThatHaveMoreXMovies(Studio[] studios, int x){
        return Arrays.stream(studios)
                .filter(studio -> studio.getNumarFilme() > x)
                .map(Studio::getNume)
    ...
    public static List<String> getAllMoviesWithActorOlderThan(Studio[] studios, int age){
        return Arrays.stream(studios)
                ...
                .filter(movie -> movie.actoriCuVarstaPeste(age))
                .map(Film::getNume)
                .collect(Collectors.toList());
    }
}
